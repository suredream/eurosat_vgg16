...
def MultiChannelImageDataGenerator(
    groups,
    batch_size,
    stats=None,
    shuffle=True,
        ...
        if shuffle:
            ...
            if batch_size > len(sample):
                sample = sample.append(
                    groups[0][0 : (batch_size - len(sample))]
                )
...
def standardiseImage(image, stats):
    """
...
def applyFlip(image, horizontal_flip=False, vertical_flip=False):
    """
...
def applyAffineTransform(
    ...
    transform_toorigin = SimilarityTransform(
        scale=(1, 1), rotation=0, translation=(-image_shape[0], -image_shape[1])
    )

    transform_revert = SimilarityTransform(
        scale=(1, 1), rotation=0, translation=(image_shape[0], image_shape[1])
    )
...
def applyCentreCrop(image, target_size):
    """
    ...
    # apply crop
    crop_image = image[
        int(midpoint[0] - math.ceil(x_crop / 2)) : int(
            midpoint[0] + math.floor(x_crop / 2)
        ),
        int(midpoint[1] - math.ceil(y_crop / 2)) : int(
            midpoint[1] + math.floor(y_crop / 2)
        ),
        :,
    ]
